...
class RelationalDatabaseVehicleRepositoryImpl(VehicleRepository):
    ...
    def get_vehicle_by_id(self, id: int) -> VehicleModel | None:
        with Session(db_engine) as session:
            ...
            if vehicle_entity:
                logger.debug(f"Method get_vehicle_by_id(), Vehicle with ID: {id} retrieved")
    ...
    def get_vehicle_by_vin(self, vin: str) -> VehicleModel | None:
        with Session(db_engine) as session:
            ...
            if vehicle_entity:
                logger.debug(f"Method get_vehicle_by_vin(), Vehicle with VIN: {vin} retrieved")
                return map_vehicle_entity_to_vehicle_model(vehicle_entity)
    ...
    def get_vehicle_by_plate(self, plate: str) -> VehicleModel | None:
        with Session(db_engine) as session:
            ...
            if vehicle_entity:
                logger.debug(f"Method get_vehicle_by_plate(), Vehicle with plate: {plate} retrieved")
                return map_vehicle_entity_to_vehicle_model(vehicle_entity)
    ...
    def get_vehicles(self) -> List[VehicleModel] | None:
        with Session(db_engine) as session:
            ...
            if vehicles:
                logger.debug(f"Method get_vehicles(), Retrieved {len(vehicles.all())} vehicles")
                return [
    ...
    def remove_vehicle_by_id(self, id: int) -> int | None:
        with Session(db_engine) as session:
            ...
            session.commit()
            logger.debug(f"Method remove_vehicle_by_id(), Vehicle with ID: {id} removed")
    ...
    def update_vehicle(self, vehicle_update: VehicleModel, id: int):
        with Session(db_engine) as session:
            ...
            logger.debug(f"Method update_vehicle(), Vehicle with ID: {id} updated")
    ...
    def create_vehicle(self, vehicle: VehicleModel) -> VehicleModel:
        with Session(db_engine) as session:
            ...
            session.commit()
            session.refresh(vehicle_entity)
            logger.debug(f"Method create_vehicle(), Vehicle created with ID: {vehicle_entity.id}")
            return map_vehicle_entity_to_vehicle_model(vehicle_entity)

    def get_number_of_vehicles(self) -> int:
        with Session(db_engine) as session:
            number_of_vehicles = len(session.exec(select(Vehicle)).all())
            logger.debug(f"Method get_number_of_vehicles(), Retrieved {number_of_vehicles} vehicles")
