...
class RelationalDatabaseInvitationCodeRepositoryImpl(InvitationCodeRepository):
    ...
    def get_invitation_code_by_code(self, code: str) -> InvitationCodeModel | None:
        self.logger.info(f"Method get_invitation_code_by_code(), Invitation code with code {code}")
    ...
    def get_invitation_code_by_email(self, email: str) -> InvitationCodeModel | None:
        with Session(db_engine) as session:
            ...
            if invitation_code_entity:
                self.logger.info(f"Method get_invitation_code_by_email() ,Invitation code with email {email}")
                self.logger.debug(f"Invitation code: {invitation_code_entity}")
                return map_invitation_code_entity_to_invitation_code_model(
                    invitation_code_entity
                )
    ...
    def get_invitation_code_by_code_and_email(
        ...
        with Session(db_engine) as session:
            ...
            if invitation_code_entity:
                self.logger.info(f"Method get_invitation_code_by_code_and_email(), Invitation code with code {code} and email {email}")
                self.logger.debug(f"Invitation code: {invitation_code_entity.email} {invitation_code_entity.code}")
                return map_invitation_code_entity_to_invitation_code_model(
                    invitation_code_entity
                )
    ...
    def save_invitation_code(
        ...
        self.logger.info(f"Method save_invitation_code(), Saving invitation code: {invitation_code}")
        with Session(db_engine) as session:
            ...
            self.logger.debug(f"Invitation code saved: {invitation_code_entity.email} {invitation_code_entity.code}")
            return map_invitation_code_entity_to_invitation_code_model(
                invitation_code_entity
            )

    def delete_invitation_code_by_code(self, code: str) -> None:
        self.logger.info(f"Method delete_invitation_code_by_code(), Deleting invitation code with code {code}")
        with Session(db_engine) as session:
            invitation_code_entity = session.exec(
                select(InvitationCode).where(InvitationCode.code == code)
            ).one()
